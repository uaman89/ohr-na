...
class Catalog_content extends Catalog {
   ...
   {
        if( !$this->sort ) $this->sort='id'; //move
   ...
   <table border="0" cellpadding="0" cellspacing="1">
    ...
    <?}?>
    <Th class="THead">*</Th>
    <Th class="THead">Похожие товары</Th>
   <?
    $a = $rows;
    $j = 0;
    $up = 0;
    ...
    for( $i = 0; $i < $rows; $i++ )
    {
     ...
     <td align="center" nowrap><?
       ...
       <?}
           ...
           ?><td style="font-weight:normal;"><?
           $arr_relat_prop=null;
           $arr_relat_prop = $this->GetRelatProp( $row['id'],'mod_catalog_prop_relat2' );
           //$script2 = $this->script."&task=control_relat_prop_form&amp;id_prop1=".$row['id'];
           $script2 = "/admin/index.php?module=116&id_prop=".$row['id'];
           if ( !is_array($arr_relat_prop) OR count($arr_relat_prop)==0) $this->Form->Link( $script2, $this->multi['TXT_ADD']);
           else {
             //  $this->ShowRelatProp($arr_relat_prop, $row['id'] );
               $this->Form->Link( $script2, $this->multi['TXT_EDIT']);
...
//        echo '<br />$q='.$q.' $this->Right->result='.$this->Right->result;
   ...
   // Change Request Nbr:
   // ================================================================================================
   function ShowRelatProp( $arr, $id, $complect = false)
   {
    foreach($arr as $key=>$value) {
        ...
        if(!$complect){
            ...
            ?> <div style="border-bottom: solid 1px #ACACAC; margin: 5px;">Количество: <input name="countcomplect[<?=$value['id_prop2']?>]" type="number" min="1" size="2" value="<?=$value['count']?>"> <?=$val;?></div><?
        }
    }
   ...
   // Change Request Nbr:
   // ================================================================================================
   function DelRelatPropByIdProp( $id_prop , $tbl=TblModCatalogPropRelat)
   {
        $tmp_db = new DB();
        $q = "DELETE FROM `".$tbl."` WHERE (`id_prop1`='$id_prop' OR `id_prop2`='$id_prop')";
    ...
    function DelGroupPrice($id_prop) {

        $tmp_db = new DB();
        $q = "DELETE FROM `".TblModPropGroupPrice."` WHERE `prod_id`='$id_prop'";
        $res =$tmp_db->db_Query( $q );
        //echo '<br> $q='.$q.' $res='.$res.' $tmp_db->result='.$tmp_db->result;
        if ( !$res OR !$tmp_db->result ) return false;
        return true;
    ...
    function ShowPriceForGroup() {

        $SysGroup = check_init("SysGroup","SysGroup");
        $arr = $SysGroup->GetGrpToArr( $this->user_id, $this->module );
        $def_currency = $this->Currencies->defCurrencyData['id'];
        // $db = new Db();
        if($this->id!=NULL){
            $q = "SELECT * FROM `".TblModPropGroupPrice."` WHERE `".TblModPropGroupPrice."`.`prod_id` =  '".$this->id."' ";
            ...
            if($rows>0) {
                for($i=0;$i<$rows;$i++){
                    $row =$this->Right->db_FetchAssoc();
                    $mas[$row['group_id']]= $row;
                }
            }else{

                foreach($arr as $k=>$v) {
                    if($v['id']=='1' || $v['id']=='2') continue;

                    $q = "INSERT INTO `".TblModPropGroupPrice."` (`group_id`, `prod_id`, `price`, `group_currency`) VALUES ('".$v['id']."', '".$this->id."', '0.00', '".$def_currency."')";

                    $res = $this->Right->Query( $q, $this->user_id, $this->module );
                    $mas[$v['id']]= array('group_currency'=>$def_currency, 'price' =>'0.00', 'group_id'=>$v['id'], 'prod_id'=>$this->id);
                }
            }
        ...
        ?><fieldset title="Цена для групп"><legend> Цена для групп</legend><?
        echo "<div style='background: #E5F2FF; border:1px dotted #a9a9a9'>";
        foreach($arr as $k=>$v) {
            if($v['id']=='1' || $v['id']=='2') continue;
            ?><div style="padding: 10px; border-bottom: 1px dotted #a9a9a9">
                            <div style="width: 150px; display: inline-block;">
                                <?echo "Цена для ".$v['name'];?>:
                            </div>
           <? if( $this->id!=NULL ) $this->Err!=NULL ? $group_price_currency[$v['id']]=$this->group_price_currency[$v['id']] : $group_price_currency[$v['id']]=$mas[$v['id']]['group_currency'];
            else $group_price_currency[$v['id']] = $this->group_price_currency[$v['id']];

            if( empty($group_price_currency[$v['id']]) AND $group_price_currency[$v['id']]!='0' ) $group_price_currency[$v['id']] = $def_currency;


            if( $this->id!=NULL ) $this->Err!=NULL ? $priceforgroup[$v['id']]=$this->priceforgroup[$v['id']] : $priceforgroup[$v['id']]=$mas[$v['id']]['price'];
            else $priceforgroup[$v['id']]=$this->priceforgroup[$v['id']];


            $this->Form->TextBox( 'priceforgroup['.$v['id'].']', stripslashes($priceforgroup[$v['id']]), 10 );
            $this->Form->Select($this->Currencies->listShortNames, 'group_price_currency['.$v['id'].']', $group_price_currency[$v['id']]);?>
            </div><?
    ...
    <tr>
     <td width="170">
      ...
      if ( isset($this->settings['cod_pli']) AND $this->settings['cod_pli']=='1' ) {
          ...
          $this->Form->TextBox('cod_pli', $cod_pli, 20);
      }
     ...
     <td valign="top">
        <div class="EditTable">
             ...
             if ( isset($this->settings['best']) AND $this->settings['best']=='1' ) {
                 ...
                 if( $this->id!=NULL ) $this->Err!=NULL ? $best=$this->shareprop : $best=$mas['shareprop'];
                 else $best=$this->shareprop;
                 ($best==1)? $checked = 'checked="checked"': $checked = NULL;
                 ?><input type="checkbox" id="sharepropCheck" class="uicheckbox" name="shareprop" <?=$checked?> onclick=""/>
                 <label for="sharepropCheck">Акция</label>
                 <?
...
//    }
   ...
   if ( isset($this->settings['share']) AND $this->settings['share']=='1' ) { ?>
    ...
    ?>
       ...
       <TR><TD><b>Старая цена:</b>
           <TD>
               <?
               if( $this->id!=NULL ) $this->Err!=NULL ? $val=$this->oldprice : $val=$mas['oldprice'];
               else $val=$this->barcode;
               $this->Form->TextBox( 'oldprice', stripslashes($val), 50 );?>


    <tr>
     <td colspan="2">
      <?
        $this->ShowPriceForGroup();
    ...
    if ( isset($this->settings['price']) AND $this->settings['price']=='1' ) { ?>
        <TR>
            <TD>
                <div style="display: none"><b><?=$this->multi['FLD_PRICE'];?>:</b></div>
   ...
   if ( isset($this->settings['price_levels']) AND $this->settings['price_levels']=='1' ) { ?>
       ...
       ?><tr>
            ...
            <td colspan="2">
                ...
                <fieldset title="Характеристика"><legend><img src="images/icons/params.png" alt="Параметры" title="Параметры" border="0"> Характеристика</legend>
                    <script>
                        function addTr() {


                            $('#descHH tr:last').after('<tr class="TR2"><td style="width: 45%"><input type="text" style="width: 100%" name="nameh[]"></td><td style="width: 45%"><input style="width: 100%" type="text" name="descrh[]"></td><td style="width: 10%"><input type="button" onclick="removeTr(this)" value="Удалить"></td></tr>');
                        }
                    ...
                    <?if(!isset($this->id) ) : ?>
                        <table  border="0" cellspacing="1" cellpadding="0" class="EditTable" width="100%" id="descHH">

                            <tr>
                                <th class="THead" width="45%">Название</th>
                                <th class="THead" width="45%">Описание</th>
                                <th class="THead" width="10%">Удалить</th>

                            </tr>
                            <tr class="TR2">
                                <td><input type="text" name="nameh[]"></td>
                                <td><input type="text" name="descrh[]"></td>
                                <td><input type="button" value="Удалить" onclick="removeTr(this)"></td>
                            </tr>
                    ...
                    <?else:?>
                        ...
                        $arr = $this->getCharacteristic($this->id);
                        ...
                        if($arr){
                          ?>  <table  border="0" cellspacing="1" cellpadding="0" class="EditTable" width="100%" id="descHH">
                            ...
                            foreach($arr as $k=>$v ) {
                                ...
                                <tr class="TR2">
                                    <td><input style="width:100%" type="text" name="nameh[]" value="<?=$v['name']?>"></td>
                                    <td><input style="width:100%" type="text" name="descrh[]" value="<?=$v['descr']?>"></td>
                                    <td><input type="button" value="Удалить" onclick="removeTr(this)"></td>
                                </tr>
                            <?
                            }
                        ...
                        ?></table><?
                            ...
                            <table  border="0" cellspacing="1" cellpadding="0" class="EditTable" width="100%" id="descHH">
                                ...
                                <tr>
                                    <th class="THead" width="20%">Название</th>
                                    <th class="THead" width="75%">Описание</th>
                                    <th class="THead" width="5%">Удалить</th>
                    ...
                    <?endif;?>
                        <input type="button" value="Добавить" onclick="addTr()">
...
//          var_dump($value);
    ...
    ?><tr><?
       ...
       ?><td colspan="2">
          ...
          <fieldset title="<?=$this->multi['FLD_FILES']?>"><legend><img src='images/icons/files.png' alt="Комплект" title="Комплект" border="0" /> Комплект</legend>
             <?
             $arr_relat_prop=null;

             $arr_relat_prop = $this->GetRelatProp( $this->id,'mod_catalog_set_prop' );

             $script2 = "/admin/index.php?module=112&ajax=1&id_prop=".$this->id;



              if ( !is_array($arr_relat_prop) OR count($arr_relat_prop)==0) {

                  ?><a class="openwindow" href="<?=$script2?>"><?=$this->multi['TXT_ADD']?></a><?

              }else {
                  $this->ShowRelatProp($arr_relat_prop, $this->id, true );
                  ?><a class="openwindow" href="<?=$script2?>"><?=$this->multi['TXT_EDIT']?></a><?
...
//
            ...
            //проверяем или есть комплект

           $q = "SELECT * FROM `".TblModCatalogSetProp."` WHERE `id_prop1`=".$this->id."";
           ...
           $rowsSet = $this->Right->db_GetNumRows();

           $strSet = array();
           $strSetForCount =array();

            //если есть комплект бежим по всем товарам в комплекте и собираем цену

           if($rowsSet>0) {

                for($k=0;$k<$rowsSet;$k++) {

                    $rowSet = $this->Right->db_FetchAssoc();

                    //print_r($rowSet);

                    $strSet[$rowSet['id_prop2']] = $rowSet['count'];

                    $strSetForCount[ $rowSet['id_prop2'] ]  ='';


                }

                $strPropList = implode(",", array_keys($strSet) );

               $q = "SELECT * FROM `".TblModPropGroupPrice."` WHERE `prod_id` IN (".$strPropList.")";
               ...
               $rowsSetP = $db11->db_GetNumRows();

               $arrPropPrice = array();

                if($rowsSetP>0) {

                    for($kk=0; $kk<$rowsSetP; $kk++) {

                        $rowSet = $db11->db_FetchAssoc();


                        if( !isset($arrPropPrice[$rowSet['group_id']]) ) {

                            $arrPropPrice[ $rowSet['group_id'] ] = 0;

                        }



                        if(!isset($this->countcomplect[$rowSet['prod_id']])) {

                            $this->countcomplect[$rowSet['prod_id']] = 1;

                        }

                        //echo 'group_currency = '.$rowSet['group_currency'].'<br>';
                        //echo 'price = '.$rowSet['price'].'<br>';

                        $price_group = $this->Currencies->Converting($rowSet['group_currency'], 5,$rowSet['price'] , 2);

                        //echo 'price = '.$price_group.'<br>';

                        if($rowSet['group_id'] == 5 && $rowSet['group_currency']!=5) {//для розниці и для валюті
                            $price_group = round($price_group);
                        }

                        //echo 'price = '.$price_group.'<br>';

                        $price_group = $this->countcomplect[$rowSet['prod_id']] * $price_group;
                        ...
                        //echo '<pre>';
                        //print_r($this->countcomplect);
                        //echo '</pre>';

                        //echo 'countcomplect'.$rowSet['prod_id'].' = '.$this->countcomplect[$rowSet['prod_id']].'<br>';


                        $arrPropPrice[$rowSet['group_id']] =   $arrPropPrice[$rowSet['group_id']] + $price_group;

                       // echo 'arrPropPrice'.$rowSet['group_id'].' = '.$arrPropPrice[$rowSet['group_id']].'<br>';
                    ...
                    foreach($arrPropPrice as $k=>$v) {

                        $q = "UPDATE `".TblModPropGroupPrice."` SET
                    ...
                    foreach($strSetForCount as $k=>$v) {

                        if(!isset($this->countcomplect[$k]) ){
                            $this->countcomplect[$k] = 1;
                        }


                        $q = "UPDATE `".TblModCatalogSetProp."` SET
                                `count` = '".$this->countcomplect[$k]."'
                            WHERE
                           `id_prop1` = '".$this->id."'
                           AND
                           `id_prop2` = '".$k."'
            ...
            //если нет комплекта
               ...
               foreach($this->priceforgroup as $k=>$v) {

                   $q = "UPDATE `".TblModPropGroupPrice."` SET

                       `group_currency` = '".$this->group_price_currency[$k]."',
                       `price` = '".$v."'
           ...
           $this->reloadPrice($this->id);
           $this->saveCharacteristic($this->id);
       }
       else{
          ...
          $idProdIns = $this->Right->db_GetInsertID();
          ...
          //  echo "q= ".$q." res = ".$res;
           ...
           $q = "SELECT * FROM `".TblModCatalogSetProp."` WHERE `id_prop1`=".$idProdIns."";
...
//           exit;
           if( $rowsSet>0 ) {
               ...
               for($k=0;$k<$rowsSet;$k++) {
                   ...
                   $strSet[] = $rowSet['id_prop2'];
                   $strSetForCount[ $rowSet['id_prop2'] ]  ='';

               }
               $strPropList = implode(",", $strSet);

               $q = "SELECT * FROM `".TblModPropGroupPrice."` WHERE `prod_id` IN (".$strPropList.")";

                $db111= new DB();

               $res = $db111->db_Query( $q, $this->user_id, $this->module );
               //echo '<br>$q='.$q.' $res='.$res.' $this->Right->result='.$this->Right->result;
               if( !$res  ) return false;


               $rowsSetP = $db111->db_GetNumRows();
               ...
               if($rowsSetP>0){
                   ...
                   for($kk=0; $kk<$rowsSetP; $kk++) {
                       ...
                       if($rowSet['group_id'] == 5 && $rowSet['group_currency']!=5) {
                           $price_group = round($price_group);
                       }
                   ...
                   $q =  "INSERT INTO `".TblModPropGroupPrice."` SET
                                        `group_currency` = '5',
                                        `price` = '".$v."',
                                        `group_id` = '".$k."',
                                        `prod_id` = '".$idProdIns."'";
           ...
           $this->saveCharacteristic($idProdIns);
...
//			$sizes=$this->sizes[$keys[$i]];
    ...
    function saveCharacteristic($id){
        ...
        $q = "DELETE FROM `".TblModCatalogPropCharacteristic."` WHERE `prod_id`='".$id."'";

        $res = $db->db_Query( $q );

        if( !$res  ) return false;

       if( isset($this->nameh) && isset($this->descrh) ) {

           $countNameh = count($this->nameh);
           $countDescrh = count($this->descrh);

           // echo $countNameh;
            //echo $countDescrh;
           // exit;

           if($countNameh == $countDescrh) {

               foreach($this->nameh as $k=>$v) {

                   $q = "INSERT INTO `".TblModCatalogPropCharacteristic."` (`prod_id`, `name`, `descr`, `lang_id`) VALUE ('".$id."', '".$v."', '".$this->descrh[$k]."', '".$this->lang_id."')";
    ...
    function reloadPrice($id) {
        ...
        $q = "SELECT * FROM `".TblModCatalogSetProp."` WHERE `id_prop2`=".$id."";
        ...
        if($rowsSet>0) {
            ...
            for( $i=0; $i<$rowsSet; $i++ ) {
                ...
                $this->reloadNewPrice( $row['id_prop1'] );
    ...
    function reloadNewPrice($ids) {


        $q = "SELECT * FROM `".TblModCatalogSetProp."` WHERE `id_prop1`=".$ids."";
        ...
        if($rowsSet>0) {
            ...
            if($rowsSetP>0){
                ...
                for($kk=0; $kk<$rowsSetP; $kk++) {
                    ...
                    if($rowSet['group_id'] == 5 && $rowSet['group_currency']!=5) { //if user && not UAH

                        $price_group = round($price_group);
                    }
                    $price_group = $price_group*$strSet[ $rowSet['prod_id'] ]; //умножаем на кол-во
     ...
     $u=$id_del[$i];
      ...
      if ( isset($this->settings['relat_prop']) AND $this->settings['relat_prop']=='1' ) {
       ...
       //--- delete relations with other positions for current position ---
        ...
        $res = $this->DelRelatPropByIdProp($u, TblModCatalogSet);
        if( !$res ) return false;



        $res = $this->DelGroupPrice($u);
        if( !$res ) return false;
         ...
         <TR>
          ...
          <?
            ...
            while( $el = each( $ln_arr ) )
            {
              ...
              echo "\n<tr><td><b>".$this->multi['FLD_TITLE'].":</b></td>";
              echo "\n<tr><td>";
                (isset($files_txt[$lang_id]) && $files_txt[$lang_id]['cod'] == $row['id']) ? $name = $files_txt[$lang_id]['name'] : $name = '';
              ...
              (isset($files_txt[$lang_id]) && $files_txt[$lang_id]['cod'] == $row['id']) ? $name = $files_txt[$lang_id]['text'] : $name = '';
