...
class catalog_RelatProp extends Catalog{
    ...
    function show($set=false){
        ...
        if ($this->flag == 'for_categ'){
            $name_panel = $this->multi['TXT_RELAT_PROP_FOR_CATEG'].' для';
            $name_tbl = TblModCatalogSprName;
        }
        else {
            if (!$set) {
            ...
            $name_tbl = TblModCatalogPropSprName;
        }

        $name_panel .= ' <u><strong>'.$this->Spr->GetNameByCod( $name_tbl, $this->id_prop).'</strong></u>';
        AdminHTML::PanelSubH($name_panel );
        $relatPropArr=$this->GetData();
        ?><div id="RelatPropPositionsBox" class="PanelSimpleL">
        <div style="overflow: hidden">
            <form action="/admin/index.php?module=<?=$this->module?>&id_prop=<?=$this->id_prop?>&flag=<?=$this->flag?>" method="post" name="form_mod_catalog_relat_prop" id="form_mod_catalog_relat_prop" enctype="multipart/form-data">
                ...
                if ($this->flag == 'for_categ'){
                    ...
                    $propStr = $this->showSameBlockForProp($relatPropArr, "", "Товары привязаные к категории");
    ...
    function showSameBlockForProp($relatPropArr, $name_Panel = '' , $nameText = '', $set=false){
        ...
        <script type="text/javascript">
            ...
            function reloadCatalogInner<?=$name_Panel?>($catId,$url){
                ...
                if(!$url) $url="<?=$this->script."&task=CatalogInnerShow".$name_Panel."&ajax=1&propStr="?>"+$("#propStr<?=$name_Panel?>").val()+"&id_cat="+$catId+"&propStronThisPropPanel="+$("#propStr<?=$name_Panel?>").val();
                $.ajax({
                    url : $url,
                    type : "POST",
                    beforeSend : function(){
