         ...
         <tr>
          ...
          <td valign="top">
           ...
           if ( $this->level>0 ) $this->ShowPathToLevel($this->level, NULL, $this->script );
               ...
               ?><a href="<?=$this->script?>&task=show_main_category"><?=$this->multi['TXT_SETTING_ROOT_CATALOG']?></a><?
           }
         ...
         for( $i = 0; $i < $rows; $i++ ){
          ...
          <tr class="<?=$class;?>">
           ...
           <td style="font-weight:normal;">
               <?
               $arr_relat_prop=null;
               $arr_relat_prop = $this->GetRelatProp( $row['id'],'mod_catalog_prop_relat_for_categ' );
               //$script2 = $this->script."&task=control_relat_prop_form&amp;id_prop1=".$row['id'];
               $script2 = "/admin/index.php?module=116&id_prop=".$row['id'].'&flag=for_categ';
...
//              echo $mas['cod_pli'];
     ...
     <td><script type="text/javascript">
         ...
         <div class="EditTable">
            ...
            <?
            if( $this->id!=NULL ) $this->Err!=NULL ? $show_in_menu=$this->show_in_menu : $show_in_menu=$mas['show_in_menu'];
            else $this->Err!=NULL ? $show_in_menu=$this->show_in_menu : $show_in_menu=1;
            ($show_in_menu==1)? $checked = 'checked="checked"': $checked = NULL;
            ?>
            <input id="showInMenuCheck" class="uicheckbox" type="checkbox" name="show_in_menu" <?=$checked?> value="show_in_menu" onclick=""/>
            <label for="showInMenuCheck">Отображать в меню сайта</label>
        </div>
    </td>
    </tr>
    <?/*
    ...
    <? if ( isset($this->settings['multi_categs']) AND $this->settings['multi_categs']=='1' ): ?>
       <tr><td valign="top" colspan="2">
               <div class="EditTable">
                   ...
                   <?
                   if ( $this->id!=NULL ) $this->Err!=NULL ? $val=$this->multi_categs : $val=$this->GetMultiCategsByIdCat($this->id);
                   else $val=$this->multi_categs;
                   ?>
                   <div id="tag-categories-all" class="ui-tabs-panel">
                       <?
                       $arr_categs = $this->PrepareCatalogForSelect(0, NULL, $spacer = NULL, 'back', true, false, false, false, NULL, $this->id, 0);
...
//      echo '<br>q='.$q.' res='.$res.' $this->Right->result='.$this->Right->result;
    ...
    if ( isset($this->settings['multi_categs']) AND $this->settings['multi_categs']=='1' ) {
        $res=$this->SaveMultiCategsByIdCat( $this->id, $this->multi_categs );
        if( !$res ) return false;
    }
   ...
   function CheckFields()
   {
    ...
    function down_relat_categ($table)
    {
      ...
      */
     function GetMultiCategsByIdCat( $id_item=NULL )
     {
         $db = new DB();
         $q = "SELECT `id_cat` FROM `".TblModCatalogCatMultiCategs."` WHERE `id_prop`='".$id_item."'";
         $res = $db->db_Query( $q );
      ...
      * @version 1.0, 31.03.2011
      */
     function SaveMultiCategsByIdCat( $id_item, $arr )
     {
         $db = new DB();
         $q = "DELETE FROM `".TblModCatalogCatMultiCategs."` WHERE `id_prop`='".$id_item."'";
         $res = $db->db_Query( $q );
         //echo '<br> $q='.$q.' $res='.$res.' $db->result='.$db->result;
         if( !$res OR !$db->result ) {return false;}
         ...
         $cnt = count($keys);
         for($i=0;$i<$cnt;$i++){
             $q = "INSERT INTO `".TblModCatalogCatMultiCategs."` SET
                 `id_prop`='".$id_item."',
                 `id_cat`='".$keys[$i]."'";
             $res = $db->db_Query( $q );
             //echo '<br> $q='.$q.' $res='.$res.' $db->result='.$db->result;
