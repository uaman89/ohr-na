...
class DynamicPages extends Page {
   ...
   function GetPageData($page, $lang_id=NULL)
   {
    ...
    /**
     * Class method loadSpecContent
     * Get spec content from mass treeSpecContent by cod pages
     * @param integer $cod - cod of the pages
     * @param boolean $trip_tags - kill tags or no
     * @return content of pages
     * @author Bogdan Iglinsky  <bi@seotm.com>
     * @version 1.0, 15.05.2013
     */
    function getSpecContentByCod($cod = NULL,$trip_tags = true){
    ...
    function getPartner(){
        ...
        $q = "SELECT * FROM `mod_strateg_partners` WHERE `lang_id`='".$this->lang_id."'
                        AND `visible` = '1'
            AND `img`!='' ORDER BY `move` ASC";
        $src = '/images/spr/mod_strateg_partners/';
        ...
        for($i = 0; $i < $rows; $i++){
            $partnersData[$i] = $this->db->db_FetchAssoc($res);
            if( !empty($partnersData[$i]['img']) ) {
                $partnersData[$i]['img'] = ImageK::getResizedImg($src.$this->lang_id.'/'.$partnersData[$i]['img'], 'size_width=153', 85, NULL);
                $partnersData[$i]['img'] = ImageK::getResizedImg($partnersData[$i]['img'], 'size_height=85', 85, NULL);
            }
