    ...
    case 'save':
        ...
        if ( $User->SaveUser() ){
            ...
            if ($User->id==NULL) {
                ...
                //var_dump($User->GetSysUserIdByUserLogin($User->old_email));
                $res = $User->update_user($User->GetSysUserIdByUserLogin($User->old_login), $User->group_id, $User->email, $User->enrol_date, $User->login_multi_use);
                ...
                //if change login
                if ( !empty($User->id) AND $User->old_login!=$User->login){
                    if ( !$User->ChangeUserLogin($User->old_login, $User->login) ){
