...
//echo '<br> $User->task='.$User->task.' $login='.$login;exit;
...
switch ($User->task) {
    ...
    case 'send_pass':
        ...
        if ( empty($User->phone_mob) ){
            $User->Err = $User->Err . 'Введите номер телефона!<br>';
            $User->ForgotPass();
            return false;
        }
        //if ($User->unique_email($User->email)) {
        //var_dump($User->phone_mob);
        $User->email = $User->GetUserEmailByUserLogin($User->phone_mob);
        if ( $User->email) {
            ...
            else $User->password = $User->GetUserPassword($User->phone_mob);
            if (!$User->SendNewPass()) {
            ...
            $User->Err = "Пользователь с таким номером телефона не зарегестрирован.";
            $User->LoginPage();
            return false;
        }
    ...
    case 'update':
        ...
        if ($User->SaveUser()) {
            ...
            if ( !empty($res) ){
                $q = "UPDATE `" . TblSysUser . "` SET `email`='" . $User->email . "' WHERE `id`='" . $User->Logon->user_id . "'";
                $res = $User->db->db_Query($q);
                $data = array('error'=>'' );
            }
            else {
                $data = array('error' => 'Возникла ошибка.');
            }
            exit( json_encode($data));
        }
        break;
