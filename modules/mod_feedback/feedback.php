...
switch ($FeedbackLayout->task){
  case 'send':
    ...
    if( !$FeedbackLayout->send_form() ) {
        echo "<div class='response'><h2 class='err'>".$FeedbackLayout->multi['_TXT_NO_SENT']."</h2></center></div>";
        $FeedbackLayout->show_form();
    }
    else echo "<div class='response'><h2>".$FeedbackLayout->multi['_TXT_MESS_SENT']."</h2></div";
    break;
  default:
    $FeedbackLayout->show_form();
    break;
