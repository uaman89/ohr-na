 ...
 class SystemCurrencies {
    ...
    */
    function __construct($user_id=NULL, $module=NULL, $front_back = 'front') {
        ...
        ( $front_back   !="" ? $this->front_back  = $front_back   : $this->front_back  = NULL );
    ...
    function Converting($from_currency, $to_currency, $summa, $float = 2)
    {
        ...
        if($from_currency!=$to_currency AND $from_currency!=0){
            ...
            //округляем для розниці, при конвертации с валюті в гривні
            if($this->front_back=='front') {
                $logon = check_init('UserAuthorize', 'UserAuthorize');
                $this->user_group_id = $logon->user_type;

                if (empty($this->user_group_id) || $this->user_group_id == 5) { //група "розничная цена"
                    if ($from_currency != 5 && $to_currency == 5) { // и ковертируем с валюты в гривны
                        $ret = round($ret);
                    }
                }
            }
