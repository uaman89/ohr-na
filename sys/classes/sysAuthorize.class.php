...
//end of function CreateUserHash()
    ...
    function GetSessionHash($session_id) {
        //select most new hash. Because after browser update hash will be change too.
        $q = "SELECT `hash` FROM `" . TblSysSessionHash . "` WHERE `session_id`='" . $session_id . "' ORDER BY `ses_tm` DESC LIMIT 1";
        $res = $this->db->db_Query($q);
        $rows = $this->db->db_GetNumRows();
