...
//         echo '<br>q='.$q.' res='.$res.' $db->result='.$db->result;
   ...
   function getCategoryArr() {
       $q = "SELECT *
              FROM `mod_catalog_spr_name`,`mod_catalog`
              WHERE `mod_catalog_spr_name`.`cod` = `mod_catalog`.`id`
              AND  `mod_catalog`.`level` = 0
              AND `mod_catalog_spr_name`.`cod` != 0
            ";
       ...
       $rows = $DBs->db_GetNumRows();

       $arr = array();

       if($rows>0) {
           for($i=0; $i<$rows; $i++) {
               $arr[] = $DBs->db_FetchAssoc();
           }
   ...
   function showCategory($arr, $cod=0) {
       if( !is_array($arr) || count($arr)==0)
           return false;
       ?><select name="categoryCod">
            <option value="0" <?php if($cod==0) echo " selected ";?>> </option>
       <?
       foreach($arr as $k=>$v) {
           ?><option value="<?=$v['cod']?>" <?php if($cod==$v['cod']) echo " selected ";?>><?=$v['name']?></option><?
       }
       ?></select><?
   }
