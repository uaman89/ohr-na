	...
	public function __construct($message, array $variables = NULL, $code = 0)
	{
		if (defined('E_DEPRECATED'))
		{
			// E_DEPRECATED only exists in PHP >= 5.3.0
			 $php_errors[E_DEPRECATED] = 'Deprecated';
		}
	...
	public static function handler(Exception $e)
	{
		try
		{
			...
			if ( ! headers_sent())
			{
				...
				header('Content-Type: '. $error_view_content_type.'; charset='.Kohana::$charset, TRUE, $http_header_status);
			}
			...
			if ($view_file = Kohana::find_file('views',  $error_view))
			{
				include $view_file;
			}
			else
