...
<body>
 <script type="text/javascript">
  ...
  $(document).ready(function(){
      ...
      $("a[target='_blank']").not('.nofancybox').fancybox({
...
//         $count = $this->LevelCountItem( $arr_items[ $i ][ 3 ] );
         ...
         else
         {
           ...
           else
           {
            ...
            //some hardcode for custom order statuses:
            if ( $arr_items[ $i ][ 1 ] == 106 ){ // admin/index.php?module=106 == mod_order/order.backend.php
                //var_dump($arr_items[ $i ][ 1 ]);
                $statuses = $this->GetSysSprTableData('mod_order_statuses_spr');
                //var_dump($statuses);
                ...
                if (!empty($statuses)){
                    ...
                    <li>
                        <ul>
                            <li><b> фильтр по статусам: </b></li>
                            <? foreach ($statuses as $key => $status): ?>
                                <? if ( isset($_REQUEST['fltr']) && ($_REQUEST['fltr'] == $key) ) $class = 'active'; else $class = 'item'; ?>
                                <li><a class="<?=$class?>" href="?module=<?=$arr_items[ $i ][ 1 ]?>&fltr=<?=$key?>"><?=$status?></a></li>
                            <? endforeach; ?>
                        </ul>
                    <?
                }
...
//GetSysSprTableData copypasted from order.class.php
function GetSysSprTableData($table)
{
   ...
   //echo '<br>$q='.$q.' $res='.$res.' $dbr->result='.$this->db->result;
