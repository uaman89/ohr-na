...
// End of CommentsCtrl Constructor
    ...
    function show() {
        ...
        $q = "SELECT `".TblSysModComments."`.*
              FROM ".$from." ".$lef_join." WHERE";

        if (empty($this->fltr2)) $q .= " 1"; // ...where 1
        else{
            $where = substr($where, 3); // cut first 'OR'
            $where = '('.$where.')';    // wrap all OR's statements into one group
        }

	    if($this->fltr) $q .= " AND `".TblSysModComments."`.`id_module`='".$this->fltr."'";
        if($this->fltr2) $q .= $where;
        if( !empty($this->fltr3) && $this->fltr3 != 'all' ){
            if ( $this->fltr3 == 'zero' ) $status = '0';
            else $status = $this->fltr3;
            $q .= " AND `status`='$status'";
        }
        if($this->srch) $q .= " AND `".TblSysModComments."`.`name` LIKE '%".$this->srch."%' ";
         ...
         */
        if ($rows == 0) echo '<tr><td colspan="8"> поиск не дал результатов </td></tr>';
            ...
            <td colspan="10" valign="top" style="height: 60px;background:#E5F2FF;padding:15px 0 0 20px;">
                <table cellpadding="3" cellspacing="5">
                    <tr>
                        <td>Статус:</td>
                        <td>
                            <select name="fltr3" style="border: solid 1px #BFBFBF;">
                                <?
                                if ( empty($this->fltr3) ) $this->fltr3 = 'all';
                                $arr_status = array('all'=>'все', '2' => 'Не проверенные', '1' => 'Отображать на сайте', 'zero' => 'Не отображать');
                                foreach ($arr_status as $key=>$val){
                                    if ( $key == $this->fltr3 ) $sel = "selected";
                                    else $sel = '';
