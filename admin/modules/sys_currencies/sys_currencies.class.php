   ...
   <div><?
	...
	for( $i = 0; $i < $rows; ++$i )
	{
	   ...
	   if( $i >=$this->start && $i < ( $this->start+$this->display ) )
	   {
		  ...
		  <td align="left">&nbsp;
			<?
            echo $this->ShowPrice(100, true, 2, $this->def_curr['id']);
            echo ' = ';
            echo $this->ShowPriceWithConverting(100, true, 2, $this->def_curr['id'], $row['id']);
            ?>
	      </td>
		  <td align="center">
		   <?
   ...
   $ln_arr = $ln_sys->LangArray( _LANG_ID );
	...
	else
	{
	  ...
	  while( $el = each( $ln_arr ) )
	  {
       ...
       include(SITE_PATH.'/modules/mod_catalog/backend/catalog_content.class.php');
       $this->Catalog_content = new Catalog_content();


       //$q = "SELECT `id_prop1` FROM  `mod_catalog_set_prop` WHERE  `id_cat` ='393'";
       $q = "SELECT DISTINCT `id_prop1` as `id` FROM  `mod_catalog_set_prop` "; // select all sets in all categories
       ...
       foreach($arrBuf as $k=>$v){

           $this->Catalog_content->reloadNewPrice($v['id']);
       }
