    ...
    function ShowContentHTML($result=NULL)
    {
        ...
        <table border="0" cellpadding="0" cellspacing="1" width="100%">
         <tr>
          ...
          <?
          if( $this->useshort==1 ){
              $short_title = !empty($this->short_title) ? $this->short_title : $this->Msg_text['_FLD_SHORT_NAME'];

          ?>
          <Th class="THead"><? $this->Form->LinkTitle($this->script."&sort=short", $short_title);?></Th>
          <?}
          if( $this->usehref==1 ){?>
...
//                  echo $row['cod'].'<br />node'.$row['node'];
   ...
   function edit()
   {
    ...
    $this->field_type = mysql_field_type($this->Rights->result,1);
     ...
     if($this->spr=="mod_menu" || $this->spr=="mod_menu_text") {

     ?>
     <tr>
         <td><b>Привязать к категории:</b></td>
         <td>
             <?
             $arrCat = $this->getCategoryArr();
             if( $this->id!=NULL ){
                 $this->Err!=NULL ? $val=0 : $val=$mas['category'];
             }
             else{
                 $val=0;
             }
             $this->showCategory($arrCat,$val);


             //$arr_levels = $this->GetStructureInArray($this->spr, 0, NULL, $this->Msg_text['_TXT_ROOT_LEVEL'], '&nbsp;');
            // $this->Form->Select( $arrCat, 'categoryCod', 1);
             ?>
         </td>
     </tr>
     <?
...
//       echo '<br>mas=<pre>'; print_r($arr_levels);echo '</pre>';
       ...
       while( $el = each( $ln_arr ) )
       {
             ...
             $Panel->WriteItemHeader( $lang );
                ...
                if( $this->useshort==1) {
                    echo "\n <tr>";
                    echo "\n<td><b>";
                    if (!empty($this->short_title)) echo $this->short_title;
                    else echo $this->Msg_text['_FLD_SHORT_NAME'];
                    echo ':</b>';
                    echo "\n <td>";
                    if( $this->id!=NULL ) $this->Err!=NULL ? $short=$this->short[$lang_id] : $short = $row['short'];
                    else $short=$this->short[$lang_id];
                    $this->Form->TextBox( 'short['.$lang_id.']', stripslashes($short), 80 );
                }
    ...
    function getCategoryForSubLevel() {
            $q = "SELECT
                      `mod_catalog`.`id`,
                      `mod_catalog_spr_name`.`name`
                       FROM
                       `mod_catalog`,
                        `mod_catalog_spr_name`
                         WHERE `mod_catalog`.`level`=0
                          AND `mod_catalog`.`id`!=0 ";
    ...
    while( $el = each( $ln_arr ) )
    {
       ...
       if( empty($translit) ) {
           ...
           if(!$this->id){
               ...
               $translit = $Crypt->GetTranslitStr(strip_tags($name));
               if(empty($translit )) $translit = $Crypt->GetTranslitStr(strip_tags($short));
           }
       }
       //echo '<br>$this->img=';
       //print_r($this->img);
       ...
       if( $rows>0 )   //--- update
       {
          ...
          if( $this->spr=='mod_menu' or $this->spr=='mod_menu_text' ) $q = $q.", `category`='".$this->categoryCod."'";
         ...
         if ( !empty($_FILES["image"]["name"][$lang_id]) ) {
           if ( isset($_FILES["image"]) && is_uploaded_file($_FILES["image"]["tmp_name"][$lang_id]) && $_FILES["image"]["size"][$lang_id] ){
            $filename = $_FILES['image']['tmp_name'][$lang_id];
               $arr_filename = explode('.',$_FILES['image']['name'][$lang_id]);
               $strlen_arr_filename = count($arr_filename);
               $ext = $arr_filename[$strlen_arr_filename-1];
            ...
            if (!in_array($ext, $valid_types)) {
              ...
              //if (($size) && ($size[0] < $max_image_width) && ($size[1] < $max_image_height)) {
                ...
                if( !empty($name_no_ext) ) {
                    $Crypt = check_init('Crypt', 'Crypt');
                    $name_no_ext = $Crypt->GetTranslitStr(strip_tags($name_no_ext));
                }
